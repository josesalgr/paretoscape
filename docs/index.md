...
actions are mutually exclusive, at most one action can be selected in
each planning unit. A planning unit may also remain without
intervention.
...
values represent losses, and zero indicates no change. If \\b\_{if}\\ is
the baseline amount of feature \\f\\ in planning unit \\i\\, the final
amount after selecting action \\a\\ is

\\\text{final amount}\_{iaf} = b\_{if} + \Delta\_{iaf},\\

where \\\Delta\_{iaf}\\ is the value supplied in the `delta` column. The
...
labels points using `run_id`, whereas functions that extract or map
stored plans generally use `solution_id`.
[`get_runs()`](https://josesalgr.github.io/multiscape/reference/get_runs.md)
provides the link between the run configurations and the solutions that
...
provides a complementary decision-space summary of which planning-unit
states recur or vary across the alternatives considered.
